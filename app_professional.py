...
# Page config for professional dashboard
st.set_page_config(
    page_title="BiteBalance Pro - AI Control Tower",
    page_icon="🏆",
    layout="wide",
...
# Professional CSS with sidebar animations
st.markdown("""
<style>
...
def render_control_tower_sidebar():
    """Render the professional Control Tower sidebar"""
    ...
    with st.sidebar:
        ...
        if allergies or dietary_pref != "None":
            ...
            for constraint in constraints:
                st.markdown(f'<span class="health-profile-tag">{constraint}</span>', unsafe_allow_html=True)
        ...
        if st.session_state.meal_history:
            for i, meal in enumerate(st.session_state.meal_history[-3:]):  # Show last 3
                with st.expander(f"{meal['food']} - {meal['timestamp'][:10]}"):
                    st.write(f"**Choice:** {meal['winner']}")
                    st.write(f"**Persona:** {meal['persona']}")
                    st.write(f"**Confidence:** {meal['confidence']}%")
        else:
...
def analyze_food_with_persona(food_input, persona, health_profile):
    ...
    # Base options (same as before but with persona weighting)
...
def main():
    ...
    selected_persona, show_reasoning, confidence_threshold = render_control_tower_sidebar()
